...
import { businessCourseDetail } from '@/lib/content/static-content';

export default function BusinessCoursePage() {
  return (
    <div className="flex flex-col">
      {/* ヒーロー */}
      <section className="relative py-24 md:py-40 bg-gradient-to-br from-[#1A4971] via-[#234A6D] to-[#2C5F8F] overflow-hidden">
        {/* 背景装飾 */}
          ...
          >
            ...
            >
              <h2 className="text-2xl md:text-4xl font-bold text-white mb-6 leading-tight">
                {businessCourseDetail.hero.headline}
              </h2>
              <p className="text-lg md:text-xl text-blue-50 leading-relaxed">
                {businessCourseDetail.hero.subheadline}
              </p>
            </motion.div>
            ...
            >
              <a
                href="#schedule"
                className="group inline-flex items-center gap-3 bg-white text-[var(--color-primary)] px-8 py-4 rounded-full font-bold text-lg shadow-xl hover:shadow-2xl hover:scale-105 transition-all duration-300"
              >
                <span>開催予定を見る</span>
      ...
      <section id="schedule" className="py-16 md:py-24 bg-gradient-to-br from-blue-50 via-white to-slate-50">
        <div className="container mx-auto px-6 sm:px-10 max-w-[1200px]">
          ...
          >
            ...
            {businessCourseDetail.upcomingEvents.length === 0 ? (
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true, amount: 0.3 }}
                ...
                <div className="bg-white rounded-2xl p-8 md:p-12 shadow-[0_4px_20px_rgba(0,0,0,0.08)] border-2 border-blue-100 text-center">
                  <div className="mb-6">
                    ...
                    <p className="text-base text-[var(--color-text)] leading-relaxed mb-8">
                      個別相談やご希望の日程での開催も承っております。<br className="hidden sm:block" />
              ...
              <>
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  {businessCourseDetail.upcomingEvents.map((event, index) => (
                <motion.div
                  key={event.id}
                  initial={{ opacity: 0, y: 20 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  ...
                  className="bg-white rounded-2xl p-6 md:p-8 shadow-[0_4px_20px_rgba(0,0,0,0.08)] border-2 border-blue-100 hover:border-[var(--color-primary)]/30 transition-colors duration-300"
                >
                  {/* ステータスバッジ */}
                  <div className="flex items-center justify-between mb-4">
                    <span className="inline-block bg-[var(--color-accent)] text-white text-xs font-bold px-3 py-1 rounded-full">
                      {event.status}
                    </span>
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="block text-center py-3 px-6 bg-[var(--color-primary)] hover:bg-[#15395A] text-white rounded-full font-bold text-sm transition-all duration-300 hover:scale-105 hover:shadow-lg"
                  >
                    お申し込みはこちら
                  </a>
                </motion.div>
        ...
        <div className="container mx-auto px-6 sm:px-10 max-w-[1200px]">
          ...
          >
            ...
            <ul className="grid md:grid-cols-2 gap-4">
              {businessCourseDetail.recommended.map((item, index) => (
                <motion.li
                  key={index}
                  ...
                  className="flex items-start gap-3 bg-gradient-to-br from-blue-50 to-white rounded-xl p-5 border border-blue-100 shadow-sm text-base text-[var(--color-text)]"
                >
      ...
      <section className="py-20 md:py-32 bg-[#F0F1F3]">
        <div className="container mx-auto px-6 sm:px-10 max-w-[1200px]">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {businessCourseDetail.features.map((feature, index) => (
              <motion.div
                key={index}
                initial={{ opacity: 0, y: 30 }}
                whileInView={{ opacity: 1, y: 0 }}
          ...
          >
            <div className="bg-gradient-to-br from-primary/10 to-white rounded-2xl p-8 md:p-10 shadow-[0_4px_20px_rgba(0,0,0,0.08)]">
              <div className="flex items-center gap-3 mb-6">
                <span className="inline-block bg-[var(--color-accent)] text-white text-sm font-bold px-4 py-2 rounded-full">
                  無料
                </span>
                <h3 className="text-2xl md:text-3xl font-bold text-[var(--color-primary)]">
                  {businessCourseDetail.process.initial.title}
                </h3>
              </div>
              <div className="space-y-4">
                {businessCourseDetail.process.initial.steps.map((step, index) => (
                  <div key={index} className="flex items-start gap-4">
                    ...
                    <div>
                      <h4 className="font-bold text-[var(--color-primary)] mb-1">{step.title}</h4>
          ...
          >
            <div className="bg-white rounded-2xl p-8 md:p-10 shadow-[0_4px_20px_rgba(0,0,0,0.08)] border-2 border-primary">
              <h3 className="text-2xl md:text-3xl font-bold text-[var(--color-primary)] mb-4">
                {businessCourseDetail.process.recurring.title}
              </h3>
              <p className="text-base text-[var(--color-text)] mb-6">
                {businessCourseDetail.process.recurring.description}
              </p>
              <ul className="space-y-3">
                {businessCourseDetail.process.recurring.support.map((item, index) => (
              ...
              <p className="text-sm text-[var(--color-text-light)] mt-6">
                {businessCourseDetail.process.recurring.note}
              </p>
            </div>
          </motion.div>
        </div>
      ...
      <section className="py-20 md:py-32 bg-[#F0F1F3]">
        <div className="container mx-auto px-6 sm:px-10 max-w-[1000px]">
          ...
          <motion.div
            ...
            viewport={{ once: true, amount: 0.3 }}
            transition={{ duration: 0.7 }}
            className="bg-white rounded-2xl p-8 md:p-10 shadow-[0_4px_20px_rgba(0,0,0,0.08)]"
          >
            <div className="grid md:grid-cols-2 gap-8 mb-6">
              <div>
                ...
                <p className="text-3xl md:text-4xl font-black text-[var(--color-accent)]">
                  {businessCourseDetail.pricing.initial}
                </p>
              </div>
              <div>
            ...
            <div className="border-t pt-6 space-y-2">
              <p className="text-sm text-[var(--color-text-light)]">
                {businessCourseDetail.pricing.note}
              </p>
              <p className="text-sm text-[var(--color-text-light)]">
                {businessCourseDetail.pricing.corporate}
              </p>
            </div>
          </motion.div>
        </div>
        ...
        <div className="container mx-auto px-6 sm:px-10 max-w-[1200px]">
          ...
          >
            <ul className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
              {businessCourseDetail.outcomes.map((outcome, index) => (
                <motion.li
                  key={index}
                  initial={{ opacity: 0, scale: 0.95 }}
                  whileInView={{ opacity: 1, scale: 1 }}
